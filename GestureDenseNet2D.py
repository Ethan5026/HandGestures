...
class GestureDenseNet2D:
    ...
    def build_model(self, num_classes):
        model = nn.Sequential(
            ...
            Dense2DBlock(16, growth_rate=12, num_layers=3),
            TransitionLayer2D(52, 32),
            Dense2DBlock(32, growth_rate=8, num_layers=2),
    ...
    def train(self, trainingData, trainingLabels, epochs=10, batch_size=64):
        ...
        X = torch.tensor(np.array(trainingData), dtype=torch.float32).reshape(-1, 2, 1, 21).to(self.device)
    ...
    def predict(self, testData):
        self.model.eval()
        with torch.no_grad():
            X = torch.tensor(np.array(testData), dtype=torch.float32).reshape(-1, 2, 1, 21).to(self.device)
            logits = self.model(X)
